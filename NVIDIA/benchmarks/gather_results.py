...
SYSTEM='LambdaDualBasic'
NUM_RUN = 3
PATH_RESULTS = '/home/ubuntu/benchmarks/mlperf'

# tasks = ['gnmt', 'maskrcnn', 'minigo', 'resnet', 'single_stage_detector_LambdaDualBasic', 'transformer']
tasks = ['single_stage_detector']
throughput_names = ['samples/sec']
...
for task, t_name in zip(tasks, throughput_names):  
    ...
    for file in os.listdir(task_path):
        if file.endswith(".log"):
            ...
            t2solution = single_stage_detector_T2S(filename)
            tpsec = single_stage_detector_TPS(filename)
